...
HTML pages now include header meta for CSS
Server GET method now sends CSS on request
...
'''
    ...
    def do_GET(self):
        ...
        elif name in self.pokemon_dictionary:
            self.send_response(http.HTTPStatus.OK)
    ...
    def do_POST(self):
        ...
        if name in self.pokemon_dictionary:
            ...
            self.name_to_object(name).status_line = '''<p class="status_line">Updated</p>'''
            self.send_response(http.HTTPStatus.OK)
            self.send_header("Content-type", 'text/html')
            self.end_headers()
            html = (self.boilerplate_prefix
            ...
            self.name_to_object(name).status_line = ""
